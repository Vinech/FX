...
@synthesize productname=_productname,open=_open,high=_high,low=_low,volume=_volume,id=_id,time=_time,last=_last;

-(Personaldetail *)initWithproductname:(NSString *)theproductname open:(NSString *)theopen high:(NSString *)thehigh low:(NSString *)thelow volume:(NSString *)thevolume id:(NSString *)theid time:(NSString *)thetime last:(NSString *)thelast
{
self = [super init];
if (self) {
...
+(Personaldetail *)findpersonaldetailwithproductname:(NSString *)theproductname{
    ...
    if (result == SQLITE_OK) 
    {
        ...
        if (SQLITE_ROW == sqlite3_step(stmt)) 
        {
            ...
            const unsigned char * userlast=sqlite3_column_text(stmt,7);
...
//            }
            ...
            if (userlast == nil) 
            {
                ...
                last = [NSString stringWithUTF8String:(const char *)userlast];
            }
            ...
            Personaldetail *user=[[Personaldetail alloc]initWithproductname:[NSString stringWithUTF8String:(const char *)userproductname] open:open high:high low:low volume:volume id:id time:time last:last];
...
+(int)upDateWithOpen:(NSString *)theOpen High:(NSString *)theHigh Low:(NSString *)theLow Vlume:(NSString *)theVolume Time:(NSString *)theTime 
             Last:(NSString *)last fromid:(NSString *)theid{
    sqlite3 * db = [dataBase openDB];
    sqlite3_stmt * stmt = nil;
    sqlite3_prepare_v2(db,"update product set open = ?,high = ?,low = ?,volume = ?,time = ? last = ? where id = ?", -1, &stmt, nil);
    ...
    sqlite3_bind_text(stmt, 7,[theid UTF8String], -1, nil);
    sqlite3_bind_text(stmt, 5,[theTime UTF8String], -1, nil);
    sqlite3_bind_text(stmt, 6,[last UTF8String], -1, nil);
